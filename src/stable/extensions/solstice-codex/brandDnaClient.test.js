...
(async () => {
	...
	const server = http.createServer((req, res) => {
		...
		req.on("end", () => {
			calls.push({ method: req.method, url: req.url, body: Buffer.concat(chunks).toString("utf8"), key: req.headers["x-brand-dna-key"] });
	...
	await listen(server);
	const client = new BrandDnaClient({ baseUrl: `http://127.0.0.1:${server.address().port}`, authKey: "remote-test-key", timeout: 2000 });
	...
	assert.ok(calls.every((call) => call.key === "remote-test-key"));
	...
	await close(server);
	console.log("brandDnaClient.test.js: 17/17 checks passed");
