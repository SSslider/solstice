...
(function () {
	...
	function automaticActivationHtml(draft) {
		const retry = draft.automatic_activation || {};
		const attempts = Number(retry.attempts || 0);
		if (!attempts) return "";
		const maximum = Number(retry.max_attempts || 3);
		const label = retry.exhausted ? "AUTO RETRY STOPPED · MANUAL ONLY" : "AUTO RETRY";
		return `<div class="retry-state ${retry.exhausted ? "exhausted" : ""}"><strong>${label}</strong><span>${attempts}/${maximum}</span>${retry.last_error ? `<small>${esc(retry.last_error)}</small>` : ""}</div>`;
	}
	function skillCards() {
