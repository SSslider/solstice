...
(async () => {
	...
	const server = http.createServer((req, res) => {
		...
		res.end(`<!doctype html><html><head><title>Artifact fixture</title><meta name="description" content="Artifact E2E"><meta name="viewport" content="width=device-width"><style>body{margin:0;font:18px/1.5 system-ui;background:#101521;color:white}main{min-height:1800px;padding:40px}h1{font-size:48px}h2{font-size:30px}a,button{box-sizing:border-box;display:inline-flex;align-items:center;min-height:48px;padding:12px 16px;font:inherit;color:white}button{background:#3459d8;border:0;border-radius:8px}</style></head><body><nav><a href="#proof">Proof</a></nav><main><h1>Real browser evidence</h1><button id="menu">Toggle details</button><p id="state">closed</p><section id="proof" style="margin-top:1100px"><h2>Delivery proof</h2></section></main><script>document.querySelector('#menu').onclick=()=>document.querySelector('#state').textContent='open'</script></body></html>`);
	});
