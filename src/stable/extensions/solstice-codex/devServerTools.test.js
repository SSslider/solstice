...
async function main() {
	...
	const bridge = new DevServerToolBridge({
		list: () => listOwnedDevServers(bridgeWorkspace, new Map()),
		stop: (id) => stopOwnedDevServer(bridgeWorkspace, new Map(), id),
	});
	const env = await bridge.start();
	try {
		const listResult = await requestTool("dev-server-list", {}, env);
		ok(listResult.ok && listResult.servers.length === 1 && listResult.servers[0].pid === 404, "loopback solstice/dev-server-list returns the live owned server");
		...
		const stopResult = await requestTool("dev-server-stop", { id: "workspace" }, env);
		ok(stopResult.ok && stopResult.pid === 404 && bridgeWorkspace.stopCalls === 1, "loopback solstice/dev-server-stop calls the IDE handler");
		await assert.rejects(() => requestTool("dev-server-stop", { id: "workspace" }, env), /owned_server_not_running/);
