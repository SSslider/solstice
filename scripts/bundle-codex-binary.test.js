...
assert.ok(parsed[0] > 0 || parsed[1] >= 144, `bundled Codex ${parsed.join(".")} is too old for GPT-5.6`);
...
assert.match(source, /does not satisfy GPT-5\.6 minimum \(>=0\.144\.0\)/);
assert.match(verifier, /\["gpt-5\.6-sol", "gpt-5\.6-terra"\]/);
...
console.log("bundle-codex-binary.test.js: 15/15 checks passed");
